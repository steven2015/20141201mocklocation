...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    private void handleLocationChanged(Location location, boolean passive) {
		 ...
		 * Steven
		 */
        this.cheatLocation(myLocation, passive);
	...
	private void cheatLocation(final Location location, final boolean passive){
		if(passive == false){
			...
			}else if(LocationManager.GPS_PROVIDER.equals(location.getProvider()) || LocationManager.FUSED_PROVIDER.equals(location.getProvider())){
				this.lastLocation = new Location(location);
			}
			if(this.lastLocation == null || location.getTime() - this.lastLocation.getTime() > 30){
				this.lastLocation = new Location(location);
			}
		}
	}
	private void log(final String message, final Location location){
		Log.e("steven", message + " " + location.getProvider() + " " + location.getLatitude() + "," + location.getLongitude() + " " + location.getAccuracy() + " " + new SimpleDateFormat("HH:mm:ss", Locale.US).format(new Date()));
	}
	private double floatToDouble(final float f){
		return Double.parseDouble(String.valueOf(f));
	}
	@Override
	public List<String> getActualProviders(){
		final List<String> providers = new ArrayList<String>(this.actualLocations.keySet());
		Collections.sort(providers);
		return providers;
	}
	@Override
	public Location getActualLocation(final String provider){
		return this.actualLocations.get(provider);
	}
	@Override
